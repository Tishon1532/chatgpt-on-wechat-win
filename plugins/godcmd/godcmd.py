...
ADMIN_COMMANDS = {
    ...
    "set_openai_api_key": {
        "alias": ["set_openai_api_key", "设置私有密钥"],
        "args": ["api_key"],
        "desc": "设置你的OpenAI私有api_key",
    },
    "set_gpt_model": {
        "alias": ["set_gpt_model", "设置私有模型"],
        "desc": "设置你的私有模型",
    },
    "model": {
        "alias": ["model", "模型"],
    ...
    "reset_openai_api_key": {
        "alias": ["reset_openai_api_key", "重置密钥"],
        "desc": "重置为默认的api_key",
    },
    "reset_gpt_model": {
        "alias": ["reset_gpt_model", "重置模型"],
        "desc": "重置你的私有模型",
    },
    "scanp": {
        "alias": ["scanp", "扫描插件"],
        ...
        if content.startswith("#"):
            ...
            if any(cmd in info["alias"] for info in COMMANDS.values()):
                ...
                if cmd == "auth":
                    ...
                    if bottype in [const.OPEN_AI, const.CHATGPT, const.CHATGPTONAZURE, const.LINKAI,const.CLAUDEAPI,const.XUNFEI,const.BAIDU, const.GEMINI,const.DEEPSEEK,const.COZE, const.QWEN, const.GEMINI, const.ZHIPU_AI]:
                ...
                if isadmin:
                    ...
                    if cmd == "stop":
                        ...
                        if bottype in [const.OPEN_AI, const.CHATGPT, const.CHATGPTONAZURE, const.LINKAI,const.CLAUDEAPI,const.XUNFEI,const.BAIDU, const.GEMINI,const.DEEPSEEK,const.COZE, const.QWEN,const.ZHIPU_AI, const.MOONSHOT]:
                    ...
                    #20231106添加群聊转发
                        ...
                        from plugins.admin.admin import open_admin_mode,close_admin_mode
                        if args[0]== "打开":
                            print(args[0])
                            ok, result = True,open_admin_mode()
                        elif args[0]== "关闭":
                            ok, result = True,close_admin_mode()
                    elif cmd == "updatep":
                        if len(args) != 1:
                            ok, result = False, "请提供插件名"
                        else:
